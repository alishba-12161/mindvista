...
1. **Ingestion** — Content is split into 500-word overlapping chunks. Each chunk is embedded using `all-MiniLM-L6-v2` and stored in a FAISS vector index.

2. **Retrieval** — The user's question is embedded with the same model. FAISS finds the top-4 most similar chunks using cosine similarity.

3. **Generation** — Retrieved chunks are passed to Groq's LLM with a strict prompt that instructs it to answer only from the provided context.

4. **Fallback** — If no chunks meet the similarity threshold (0.25), the system returns: *"I don't have enough information to answer that."* without calling the LLM.
